...
This produces the captions under the specified ``output_dir``. You can skip this step by using provided caption results in ``./data/{dataset_name}/captions/videollama3_json_results/``.

2. After you have the frame captions, you can run first-round scoring with Llama3.1-8B by ``bash scripts/query_llm_vad.sh`` Note that you need to adjust the paths to make sure you are using the correct captions generated by certain VLMs. 
